fn evaluate_expression(expr: &Expr, variables: &mut HashMap<String, i32>) -> i32 {
    match expr {
        ...
        Expr::Mul(left, right) => evaluate_expression(left, variables) * evaluate_expression(right, variables),
        Expr::Div(left, right) => evaluate_expression(left, variables) / evaluate_expression(right, variables),
    }
}
